...
class tsRateLimiter:
    ...
    def _yfinance_api_slow_start(self) -> None:
        ...
        app_logger.warning("🚨 Entering conservative mode - Adding %d fake events", YFINANCE_API_COOLDOWN_FAKE_EVENTS)
    ...
    def ratio_allows(self) -> bool:
        ...
        with self.lock:
            if self.yfinance_408_hit is not None:
                cooldown_over: bool = self.yfinance_408_hit + timedelta(seconds=YFINANCE_API_RATE_LIMIT_COOLDOWN_SECONDS) < datetime.now()
